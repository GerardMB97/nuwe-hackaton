export default {
  nameError: 'Introduzca un nombre válido',
  emailError: 'Introduzca un correo existente',
  passwordError: 'La contraseña debe tener 7 carácteres y alemons 1 número',
  phoneError: 'El número debe contener 9 dígitos',
  addressError: 'Introduzca una dirección válida',
  countryError: 'Introduzca un país válido',
  cardError: 'Debe contener 16 dígitos',
  secretError: 'Debe contener 3 dígitos'

};
